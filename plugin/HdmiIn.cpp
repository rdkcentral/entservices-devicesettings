...
uint32_t HdmiIn::GetHDMIInNumberOfInputs(int32_t &count) {
    ...
    DSLOG_INFO("result=%d, count=%d", result, count);
...
uint32_t HdmiIn::GetHDMIInAVLatency(uint32_t &videoLatency, uint32_t &audioLatency) {
    ...
    DSLOG_INFO("result=%d, videoLatency=%u, audioLatency=%u", result, videoLatency, audioLatency);
...
uint32_t HdmiIn::GetHDMIInAllmStatus(const HDMIInPort port, bool &allmStatus) {
    ...
    DSLOG_INFO("result=%d, port=%d, allmStatus=%s", result, port, allmStatus ? "true" : "false");
    return result;
}
...
uint32_t HdmiIn::GetHDMIInEdid2AllmSupport(const HDMIInPort port, bool &allmSupport) {
    ...
    DSLOG_INFO("result=%d, port=%d, allmSupport=%s", result, port, allmSupport ? "true" : "false");
    return result;
}
...
uint32_t HdmiIn::GetHDMIEdidVersion(const HDMIInPort port, HDMIInEdidVersion &edidVersion) {
    ...
    DSLOG_INFO("result=%d, port=%d, edidVersion=%d", result, port, edidVersion);
    return result;
}
...
uint32_t HdmiIn::GetHDMIVersion(const HDMIInPort port, HDMIInCapabilityVersion &capabilityVersion) {
    ...
    DSLOG_INFO("result=%d, port=%d, capabilityVersion=%d", result, port, capabilityVersion);
    return result;
}
...
uint32_t HdmiIn::GetVRRSupport(const HDMIInPort port, bool &vrrSupport) {
    ...
    DSLOG_INFO("result=%d, port=%d, vrrSupport=%s", result, port, vrrSupport ? "true" : "false");
    return result;
}
...
uint32_t HdmiIn::GetVRRStatus(const HDMIInPort port, HDMIInVRRStatus &vrrStatus) {
    ...
    DSLOG_INFO("result=%d, port=%d, vrrType=%d", result, port, vrrStatus.vrrType);
